...
public class MenuScreen extends AbstractGameScreen
{
	...
	private Table buildBackgroundLayer()
	{
		Table layer = new Table();
		// + Background
		imgBackground = new Image(skinPusheen, "background");
		layer.add(imgBackground);
		return layer;
	}
	
	// makes image of some genericPowerups and a huge Pusheen, which are both drawn on
	// top of the background layer.  the positions of each actor are explicitly
	// set to certain coordinates by calling setPosition() on each actor.
	private Table buildObjectsLayer()
	{
	...
	private Table buildLogosLayer()
	{
		...
		layer.left().top();
		// + Game Logo
		imgLogo = new Image(skinPusheen, "logo");
		layer.add(imgLogo);
		layer.row().expandY();
		// + Info Logos
		imgInfo = new Image(skinPusheen, "info");
		layer.add(imgInfo).bottom();
		if(debugEnabled) layer.debug();
		return layer;
	}
	...
	private Table buildControlsLayer()
	{
		...
		layer.right().bottom();
		// + Play Button
		btnMenuPlay = new Button(skinPusheen, "play");
		layer.add(btnMenuPlay);
		btnMenuPlay.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
		...
		// + Options Button
		btnMenuOptions = new Button(skinPusheen, "options");
		layer.add(btnMenuOptions);
		btnMenuOptions.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
