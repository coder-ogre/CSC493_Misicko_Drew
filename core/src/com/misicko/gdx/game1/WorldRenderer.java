...
public class WorldRenderer 
{
   ...
   //passing in SpriteBatch here is optional, since it is a global variable
	...
	private void renderGuiForSuperCookie(SpriteBatch batch)
	{
		float x = -15;
		float y = 30;
		float timeLeftForSuperCookie = worldController.level.pusheen.timeLeftForSuperCookie;
		if(timeLeftForSuperCookie > 0)
		{
			// Start icon fade in/out if the left power-up time
			// is less than 4 seconds.  The fade interfal is set
			// to 5 changes per second.
			if(timeLeftForSuperCookie < 4)
			{
				if(((int)(timeLeftForSuperCookie * 5) % 2) != 0)
				{
					batch.setColor(1, 1, 1, 0.5f);
				}
			}
			batch.draw(Assets.instance.superCookie.superCookieRegion,
				x, y, 50, 50, 100, 100, 0.35f, -0.35f, 0);
			batch.setColor(1, 1, 1, 1);
			Assets.instance.fonts.defaultSmall.draw(batch,
				"" + (int)timeLeftForSuperCookie, x + 60, y + 57);
		}
	}
